...
const Layout = ({ data, classes }) => (
  <Grid className={classes.root}>
    ...
    <Grid container direction="column" className={classes.section}>
      <Typography className={classes.title}>Welcome Hacker!</Typography>
      ...
      <Grid className={classes.section}>
        <Typography className={classes.welcome}>Example events</Typography>
        {data.events.map((event, index) => (
          <Typography className={classes.welcome} key={index}>
            Name: {event.id}
          </Typography>
        ))}
      </Grid>
      <Grid container justify="center" className={classes.buttons}>
        ...
        />
        <Button
          href="https://github.com/ethglobal/ethsanfrancisco-bounties/issues/6"
          target="_blank"
